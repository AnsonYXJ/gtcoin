...
static bool ipcScanCmd(int argc, char *argv[], bool fRelay)
{
    ...
    {
        if (boost::algorithm::istarts_with(argv[i], "gtcoin:"))
        {
...
void ipcInit(int argc, char *argv[])
{
    ...
    try {
        ...
        // Make sure we don't lose any gtcoin: URIs
        for (int i = 0; i < 2; i++)
        {
        ...
        // Make sure only one gtcoin instance is listening
        message_queue::remove(FREICOINURI_QUEUE_NAME);
        delete mq;
